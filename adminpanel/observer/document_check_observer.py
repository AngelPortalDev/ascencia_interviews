...
def notify(self, publisher):
    # Separate cache key for document verification tracking
    doc_verification_cache_key = f"doc_verification_in_progress_{publisher.id}"
    
    if cache.get(doc_verification_cache_key):
        print(f"⚠️ Document verification already in progress for {publisher.id}, skipping...")
        return  # Stop duplicate processing

    # Mark document verification as "in progress"
    cache.set(doc_verification_cache_key, True, timeout=300)  # Lock for 5 minutes
    ...
    try:
        ...
        for item in data['data']:
            ...
            async_task("adminpanel.observer.document_check_observer.process_documents_task", 
                       file_url, publisher, API_TOKEN)

    except requests.RequestException as e:
        print(f"❌ API request failed: {e}")

    finally:
        # Remove the flag after processing to allow new verifications
        cache.delete(doc_verification_cache_key)
...
def process_documents_task(file_url, publisher, API_TOKEN):
    ...
    if cache.get(doc_verification_cache_key):
        print(f"⚠️ Document verification already running for {publisher.id}, skipping...")
        return  # Skip if it's already being processed
    ...
    if file_response.status_code != 200:
        print(f"❌ Failed to download file: {file_url}")
        cache.delete(doc_verification_cache_key)  # Unlock if failed
        return
    ...
    cache.delete(doc_verification_cache_key)  # Remove the lock after processing
